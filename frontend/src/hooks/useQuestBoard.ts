...
export const useQuestBoard = () => {
  ...
  const getAcceptanceForQuest = useCallback(
    (quest: Quest): QuestAcceptance | null => {
  ...
  const dependencyStatusByQuestId = useMemo(() => {
    ...
    quests.forEach(quest => {
      const dependencyType = (
        quest.dependency_type ?? 'Independent'
      ).toLowerCase();
      const dependsOnRefs = quest.depends_on ?? [];
      const hasMissingChainedDependencies =
        dependencyType === 'chained' && dependsOnRefs.length === 0;
      ...
      if (dependencyType === 'blocked') {
        ...
        reason =
          ...
          (unresolved.length > 0
            ? `Requires: ${unresolved.join(', ')}`
            : 'Blocked by dependency rules.');
      } else if (hasMissingChainedDependencies) {
        blocked = true;
        reason =
          quest.unlock_condition ??
          'Locked: chained quest is missing dependency links.';
  ...
  const getDependencyStatus = useCallback(
    (quest: Quest): DependencyStatus =>
      dependencyStatusByQuestId.get(quest.id) ?? {
        blocked: false,
        unresolved: [],
        reason: null,
      },
  ...
  const getQuestLevel = useCallback(
    (quest: Quest): number => quest.quest_level ?? quest.difficulty ?? 1,
    []
  );
  const getQuestRank = useCallback(
    (quest: Quest): string => {
  ...
  const sortedQuests = useMemo(
    () =>
      [...quests].sort((a, b) => {
        ...
        const rankDiff =
          getRankIndex(getQuestRank(a)) - getRankIndex(getQuestRank(b));
  ...
  const handleSubmitQuestPR = async (quest: Quest) => {
    const questRef = getQuestRef(quest);
    const prUrl = window.prompt(
      'Paste your GitHub PR URL (example: https://github.com/org/repo/pull/123)'
    );
    if (!prUrl) return;

    const normalized = prUrl.trim();
    const isValidGitHubPr =
      /^https:\/\/github\.com\/[^/]+\/[^/]+\/pull\/\d+$/i.test(normalized);
    if (!isValidGitHubPr) {
  ...
  const handleCancelQuest = async (quest: Quest) => {
    ...
    const confirmCancel = window.confirm(
      'Cancel this quest? You can accept it again later.'
    );
  ...
  const resolveDependencyTitle = useCallback(
    (ref: string): string => {
      const depQuest = questRefMap.get(normalizeRef(ref));
