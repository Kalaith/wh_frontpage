...
import { useTrackerStats, useFeatureRequests, useActivityFeed } from './useTrackerQuery';
import { 
...
export const useTrackerData = () => {
  ...
  const { data: topRequests, isLoading: requestsLoading } = useFeatureRequests({
    sort_by: 'votes',
    sort_direction: 'desc',
    limit: selectedProjectIds.length > 0 ? LIMITS.TOP_REQUESTS_FILTERED : LIMITS.TOP_REQUESTS_DISPLAY,
  ...
  const { data: allRecentActivity, isLoading: activityLoading } = useActivityFeed(
    selectedProjectIds.length > 1 ? LIMITS.ACTIVITY_FEED_FILTERED : LIMITS.ACTIVITY_FEED_DISPLAY,
  ...
  const filteredTopRequests = useMemo(() => {
    ...
    return filterRequestsByProjects(topRequests, selectedProjectIds, LIMITS.TOP_REQUESTS_DISPLAY);
  ...
  const stats = useMemo(() => {
    return calculateProjectStats(allFeatureRequests, selectedProjectIds, trackerStats);
