    ...
    <div className="container mx-auto px-4 py-8">
      <header className="mb-10 text-center">
        ...
        <p className="text-lg text-slate-600">
          Raid the Backlog. Unite to crush critical tech debt and major features
          across the platform.
        </p>
      </header>
      ...
      <div className="space-y-12">
        {bosses.map(boss => {
          const hpPercent =
            boss.hp_total > 0
              ? Math.max(0, (boss.hp_current / boss.hp_total) * 100)
              : 0;

          return (
            <div
              key={boss.id}
              className="max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden"
            >
              <div className="bg-red-800 text-white p-6 md:p-8 flex justify-between items-start gap-4 relative">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span
                      className={`text-xs px-2 py-1 rounded uppercase font-semibold border ${boss.phase > 1 ? 'bg-purple-800 text-purple-100 border-purple-700' : 'bg-red-800 text-red-100 border-red-700'}`}
                    >
                    ...
                    {(boss.labels ?? []).map(label => {
                      ...
                      return (
                        <span
                          key={label}
                          className="bg-red-700 text-red-100 text-xs px-2 py-1 rounded border border-red-600 shadow-sm"
                        >
                          {formattedLabel}
                        </span>
                      );
                ...
                <div className="text-right">
                  <div className="text-3xl font-mono font-bold tracking-wider">
                    {boss.hp_current.toLocaleString()} HP
                  </div>
                  <div className="text-red-200 text-sm">
                    / {boss.hp_total.toLocaleString()} Max HP
                  </div>
                  {boss.deadline && (
