    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="text-center mb-10">
        ...
        <p className="text-lg text-gray-600 max-w-3xl mx-auto">
          Plain-language quests for players, with optional RuneSage notes when
      ...
      <div className="space-y-6">
        {chains.map(chain => {
          ...
          const isRaid =
            chain.type === 'raid' || chain.labels?.includes('type:raid');
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded font-medium ${isRaid ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`}
                      >
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right hidden sm:block">
                    <div className="text-xs text-gray-500">
                      {chain.total_steps} quests
                    </div>
                    <div className="text-sm font-bold text-indigo-600">
                      {totalStepXp + chain.reward_xp} XP total
                    </div>
                  </div>
                  <svg
                ...
                <div className="border-t border-gray-100 px-6 py-5 bg-gray-50 space-y-5">
                  ...
                  <div className="space-y-4">
                    {chain.steps.map((step, index) => {
                      ...
                      const isLocked = playerRank
                        ? compareRanks(playerRank, stepRank) < 0
                        : false;

                      return (
                        <article
                          key={step.id ?? `${chain.slug}-${index}`}
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className="text-xs font-mono text-indigo-700 bg-indigo-50 px-2 py-1 rounded whitespace-nowrap">
                              +{step.xp} XP
                            </span>
                          </div>
                          ...
                          {step.rs_brief && (
                            <div className="mt-3 border border-indigo-200 bg-indigo-50 rounded p-3">
                              <p className="text-xs font-semibold text-indigo-800 mb-1">
                                RuneSage Brief (Optional)
                              </p>
                              {step.rs_brief.context && (
                                <p className="text-sm text-indigo-900">
                                  <strong>Context:</strong>{' '}
                                  {step.rs_brief.context}
                                </p>
                              )}
                              {step.rs_brief.constraints && (
                                <p className="text-sm text-indigo-900 mt-1">
                                  <strong>Constraints:</strong>{' '}
                                  {step.rs_brief.constraints}
                                </p>
                              )}
                              {(step.rs_brief.technical_notes?.length ?? 0) >
                                0 && (
                                <ul className="list-disc list-inside text-sm text-indigo-900 mt-1 space-y-1">
                                  {step.rs_brief.technical_notes?.map(
                                    (note, i) => (
                                      <li key={i}>{note}</li>
                                    )
                                  )}
                  ...
                  <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-lg p-4">
                    <div className="font-bold text-yellow-900">
                      Chain Completion Reward
                    </div>
                    <div className="text-sm text-yellow-700 space-x-3">
                      <span>+{chain.reward_xp} XP</span>
                      {chain.reward_badge_slug && (
                        <span>
                          Badge:{' '}
                          <strong>
                            {chain.reward_badge_slug.replace(/-/g, ' ')}
                          </strong>
                        </span>
                      )}
                      {chain.reward_title && (
                        <span>
                          Title: <strong>{chain.reward_title}</strong>
                        </span>
                      )}
                    </div>
                  </div>
