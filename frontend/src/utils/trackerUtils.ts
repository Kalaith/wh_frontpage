 ...
 */
export const createFeatureToProjectMap = (
  ...
  const filtered =
    selectedProjectIds.length > 1
      ? requests.filter(
          request =>
            request.project?.id &&
            selectedProjectIds.includes(request.project.id)
        )
      : requests;
...
export const filterActivitiesByProjects = (
  ...
  const filtered = activities.filter(activity => {
    if (
      activity.reference_type === 'feature_request' &&
...
export const calculateProjectStats = (
  ...
  const filteredRequests = allFeatureRequests.filter(
    request =>
  ...
  const completedCount = filteredRequests.filter(request =>
    STATUS_VALUES.COMPLETED.includes(
      request.status as 'Completed' | 'completed'
    )
  ).length;
