        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {quest.quest_code ?? `Quest #${quest.number}`}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-5">
          <div className="flex flex-wrap gap-2">
            ...
            {acceptance && (
              ...
              >
                Status:{' '}
                {acceptance.status.charAt(0).toUpperCase() +
                  acceptance.status.slice(1)}
              </span>
            )}
          </div>
          ...
          {status.blocked && (
            <div className="border border-rose-200 bg-rose-50 text-rose-800 rounded-lg p-3 text-sm">
              <strong>🔒 Locked:</strong> Prerequisites required.{' '}
              {status.reason
                ? `(${status.reason})`
                : '(Hover/Click to view missing dependencies)'}
            </div>
          )}
            ...
            <section>
                ...
                {quest.depends_on?.map((depRef, index) => {
                  ...
                  return (
                    <li key={`${quest.id}-dep-${index}`}>
                      {label}{' '}
                      <span
                        className={`text-xs ${unresolved ? 'text-rose-600' : 'text-emerald-600'}`}
                      >
              ...
              {quest.unlock_condition && (
                <p className="text-xs text-gray-600 mt-2">
                  Unlock condition: {quest.unlock_condition}
                </p>
              )}
            </section>
          )}
