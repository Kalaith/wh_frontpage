...
export const RankProgressBar: React.FC<RankProgressBarProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-sm border ${config.border} p-4 mb-6`}
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-xs text-gray-500 ml-2">
              {rankProgress.total_xp} XP • {rankProgress.completed_quests} quest
              {rankProgress.completed_quests !== 1 ? 's' : ''} completed
            </span>
          </div>
        </div>
        {rankProgress.next_rank && (
          <div className="text-xs text-gray-500">
            Next:{' '}
            <span className="font-semibold">{rankProgress.next_rank}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between mt-1.5">
        {isMaxRank ? (
          ...
          <>
            ...
            <span className="text-[10px] text-gray-400">
              {rankProgress.quests_needed > 0 &&
                `${rankProgress.quests_needed} more quest${rankProgress.quests_needed !== 1 ? 's' : ''}`}
              {rankProgress.quests_needed > 0 &&
                rankProgress.xp_needed > 0 &&
                ' • '}
              {rankProgress.xp_needed > 0 &&
                `${rankProgress.xp_needed} more XP`}
            </span>
          </>
        )}
      </div>
