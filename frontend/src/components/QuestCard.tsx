...
export const QuestCard: React.FC<QuestCardProps> = ({
  ...
  const normalizedRank = String(
    quest.rank_required ?? derivedRank
  ).toLowerCase();
  const rankBorderStyle =
    RANK_BORDER_STYLES[normalizedRank] ?? RANK_BORDER_STYLES.iron;

  const extraLabels = quest.labels
    .filter(
      l =>
        !l.name.startsWith('difficulty:') &&
        !l.name.startsWith('xp:') &&
        l.name !== 'quest'
    )
    .slice(0, 6);
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-gray-100 flex flex-col h-full border-l-4 ${statusBorderStyle}`}
    >
      <div
        className={`p-4 pb-3 border-b border-gray-100 flex justify-between items-start`}
      >
        <div className="flex-1 min-w-0 pr-2">
          ...
          {statusBadge && (
            <span
              className={`inline-block mt-2 text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide ${statusBadge.classes}`}
            >
              {statusBadge.label}
            </span>
          )}
        </div>
        <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-bold whitespace-nowrap border border-blue-100">
          {quest.xp} XP
        </span>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col gap-3">
        <div className="flex items-start gap-3">
          <div
            className="text-sm font-bold bg-gray-50 px-3 py-2 rounded-lg text-gray-700"
            title={`Quest #${quest.number}`}
          >
            {questNumber}
          </div>
          <div>
            {quest.class_fantasy && (
              <p className="text-xs text-gray-500 mt-1">
                Suggested class: {quest.class_fantasy}
              </p>
            )}
          </div>
        </div>
        ...
        {quest.specific && (
          <p className="text-sm text-gray-800">
            <strong>Specific:</strong> {quest.specific}
          </p>
        )}
        {quest.metric_target && (
          <p className="text-sm text-gray-800">
            <strong>Target:</strong> {quest.metric_target}
          </p>
        )}
        ...
        <div className="grid grid-cols-2 gap-2">
          {metaTags.map(tag => (
            <span
              key={tag}
              className="text-xs px-2 py-1 rounded border border-gray-200 bg-white/70 text-gray-700 text-center"
            >
              {tag}
            </span>
          ))}
        ...
        {(quest.in_scope?.length ?? 0) > 0 && (
          <div className="border border-emerald-200 bg-emerald-50 rounded p-2">
            ...
            <ul className="list-disc list-inside text-xs text-emerald-900 space-y-0.5">
              {quest.in_scope?.slice(0, 3).map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
        ...
        {(quest.out_of_scope?.length ?? 0) > 0 && (
          <div className="border border-orange-200 bg-orange-50 rounded p-2">
            ...
            <ul className="list-disc list-inside text-xs text-orange-900 space-y-0.5">
              {quest.out_of_scope?.slice(0, 3).map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-auto pt-2">
          <button
            ...
            className={`block w-full text-center font-semibold py-2 px-4 rounded transition-colors text-sm ${
              status === 'completed'
