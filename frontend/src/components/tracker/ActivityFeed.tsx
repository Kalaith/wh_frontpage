...
interface ActivityFeedProps {
  ...
  featureToProjectMap?: Record<
    number,
    { id: number; title: string; group_name?: string }
  >;
}
  ...
  return (
    <div className="space-y-4">
      {activities.map(activity => {
        ...
        const project =
          activity.reference_type === 'feature_request' && activity.reference_id
        ...
        return (
          <div
            key={activity.id}
            className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border-l-4 border-blue-500 p-4"
          >
            <div className="flex items-start gap-4">
              <div
                className={`w-3 h-3 rounded-full mt-2 ${getActivityColor(activity.type, activity.action)} flex-shrink-0`}
              ></div>
